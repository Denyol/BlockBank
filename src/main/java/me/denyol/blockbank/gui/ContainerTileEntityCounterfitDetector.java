...
public class ContainerTileEntityCounterfitDetector extends Container
{
	...
	public ContainerTileEntityCounterfitDetector(IInventory playerInv, TileEntityCounterfitDetector te)
	{
		...
		addSlotToContainer(new SlotCounterfitDetector(tileEntity, 0, 84, 44, tileEntity));
	...
	protected void addPlayerInventory(IInventory inventory)
	{
		...
		for (int row = 0; row < 3; row++)
        {
            ...
            {
                addSlotToContainer(new Slot(inventory, col + (row * 9) + 9, 12 + col * 18, 102 + col * 18)); // slot index e.g (row = 0, col = 0. 0*9 +9) 9 at the end for the hot bar 0-9
            }
        }
		
		for (int col = 0; col < 9; col++) // 9 slots in the hotbar
        {
            addSlotToContainer(new Slot(inventory, col, 11 + col * 18, 159));
        }
